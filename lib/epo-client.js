 ...
 *   ready to use.
 *
 *   If validation of the ePO unique identifier fails, the first parameter
 ...
 *
 *   * No value is provided for the `epoUniqueId` parameter and zero or more
 *     than 1 ePO service is found on the DXL fabric.
 *   * A value is provided for the `epoUniqueId` parameter but no ePO service
 *     matching the id is found on the DXL fabric.
...
function EpoClient (dxlClient, epoUniqueId, callback) {
  ...
  this._getEpoId = function (callback) {
    lookupEpoUniqueIdentifiers(dxlClient,
      function (error, epoUniqueIds, useEpoCommandsService) {
        ...
        that._useEpoCommandsService = useEpoCommandsService
        ...
        if (epoIds) {
          if (epoIds.indexOf(epoUniqueId) < 0) {
            lookupEpoRemoteServiceUniqueIds(dxlClient,
              function (error, epoIds) {
                if (!error && epoIds.indexOf(epoUniqueId) < 0) {
                  error = new Error('No ePO DXL services are registered with ' +
                    'the DXL fabric for id: ' + epoUniqueId)
                }
  ...
  var epoIdCallback
  if (epoUniqueId) {
    epoIdCallback = function () {
      that._isEpoUniqueIdForCommandsService(epoUniqueId,
        function (error, useEpoCommandsService) {
          that._useEpoCommandsService = useEpoCommandsService
          callback(error)
        })
    }
  } else {
    epoIdCallback = function () { that._getEpoId(callback) }
  }

  // Wait to invoke the callback function used to validate the ePO unique id
  // until a connection has been made to the DXL fabric. The validation logic
  // involves making requests to the DXL fabric, so the client must be
  // connected before the validation is attempted.
  if (dxlClient.connected) {
    epoIdCallback()
  } else {
    dxlClient.once('connect', epoIdCallback)
  }
}
...
EpoClient.prototype.help = function (callback) {
  this.runCommand(EPO_HELP_COMMAND,
    function (error, response) {
      callback(error, response ? response.join(os.EOL) : null)
    },
    {},
    OutputFormat.OBJECT)
}
...
EpoClient.prototype.runCommand = function (commandName, callback, options) {
  ...
  if (client._epoUniqueId) {
    if (client._useEpoCommandsService) {
      client._invokeEpoCommandsService(commandName, outputFormat, params,
        callback)
    } else {
      client._invokeEpoRemoteService(commandName, outputFormat, params,
        callback)
    }
  } else {
    callback(new Error(client._epoIdSearchStatus))
  }
}
