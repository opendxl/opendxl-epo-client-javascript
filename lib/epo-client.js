...
/**
 * Get the error code from a `MessageError`. Adjust the value if it
 * is negative for use in translating standard DXL broker error codes.
 * @param {MessageError} error - The DXL `MessageError`.
 * @returns {Number} - The error code (with any necessary adjustments).
 * @private
 */
...
/**
 * Determine an `Error` contains a DXL `ErrorResponse` with a
 * "service unavailable" error code.
 * @param {Error} error - The Error
 * @returns {Boolean} - `true` if the error contains a "service unavailable"
 *   `ErrorResponse`, else `false`.
 * @private
 */
function containsServiceNotAvailableErrorResponse (error) {
  return (error && error.name === 'MessageError' &&
    (normalizedErrorCode(error) === DXL_SERVICE_UNAVAILABLE_ERROR_CODE))
}
  ...
  if (client._epoUniqueId) {
    if (client._useEpoCommandsService) {
      client._invokeEpoCommandsService(commandName, outputFormat, params,
        function (error, response) {
          if (containsServiceNotAvailableErrorResponse(error)) {
            client._invokeEpoRemoteService(commandName, outputFormat, params,
              function (error, response) {
                if (response) {
                  client._useEpoCommandsService = false
                }
        ...
        function (error, response) {
          if (containsServiceNotAvailableErrorResponse(error)) {
            client._invokeEpoCommandsService(commandName, outputFormat, params,
              function (error, response) {
                if (response) {
                  client._useEpoCommandsService = true
                }
                callback(error, response)
              }
            )
