 ...
 *   the first parameter supplied to the callback contains a
 *   [DxlError]{@link external:DxlError} with failure details. On successful
 *   lookup, the array of unique identifier strings is provided as the second
 ...
 *   the first parameter supplied to the callback contains a
 *   [DxlError]{@link external:DxlError} with failure details. On successful
 *   execution of the command, the remote command output is provided as the
 *   second parameter to the callback.
 * @param {String} outputFormat - The output format for ePO to use when
 *   returning the response. The list of `output formats` can be found in the
 *   [OutputFormat]{@link module:OutputFormat} constants module.
 ...
 *   execution of the command, the remote command output is provided as the
 *   second parameter to the callback.
 * @param {Object} params - An object containing the parameters for the command.
 * @param {String} outputFormat - The output format for ePO to use when
 *   returning the response. The list of `output formats` can be found in the
 *   [OutputFormat]{@link module:OutputFormat} constants module.
 * @throws {TypeError} If the `outputFormat` is not valid.
...
EpoClient.prototype.runCommand = function (commandName, callback,
  ...
  if (this._epoUniqueId) {
    ...
    this._dxlClient.asyncRequest(request, function (error, response) {
      callback(error, response ? response.payload : null)
    })
  } else {
